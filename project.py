...
def fbconnect():
    ...
    try :
        ...
        print 'there is No email "%s"' % str(e)    


    # The token must be stored in the login_session in order to logout, let's strip out the information before the equals sign in our token
...
def createUser(login_session):
    newUser = User(name=login_session['username'], email=login_session[
                   'email'], picture=login_session['picture'], fb_id=login_session['facebook_id'])
    session.add(newUser)
    session.commit()
    try:
        user = session.query(User).filter_by(email=login_session['email']).one()
    except:
        user = session.query(User).filter_by(fb_id=login_session['facebook_id']).one()    
    return user.id
